...
# Append the dummy HTTP server with proper capitalization (daemon=False)
...
# Create a robust startup wrapper script for valid dotenv formatting
RUN echo '#!/bin/sh' > /app/start.sh && \
    echo 'cp /app/.env.example /app/.env 2>/dev/null || true' >> /app/start.sh && \
    echo '[ -n "$DISCORD_TOKEN" ] && echo "DISCORD_TOKEN=\"$DISCORD_TOKEN\"" >> /app/.env' >> /app/start.sh && \
    echo '[ -n "$WEBHOOK_URL" ] && echo "WEBHOOK_URL=\"$WEBHOOK_URL\"" >> /app/.env' >> /app/start.sh && \
    echo '[ -n "$CHAT_CHANNEL_ID" ] && echo "CHAT_CHANNEL_ID=\"$CHAT_CHANNEL_ID\"" >> /app/.env' >> /app/start.sh && \
    echo '[ -n "$CHAT_LOG_CHANNEL_ID" ] && echo "CHAT_LOG_CHANNEL_ID=\"$CHAT_LOG_CHANNEL_ID\"" >> /app/.env' >> /app/start.sh && \
